  ...
  const ListItem = ({ item }) => {
    return (
      <DashGrid style={{ height: '48px' }}>
        ...
        <DataText area='value'>
          {currency === 'EOS' ? 'Ξ ' + formattedNum(item.valueEOS) : formattedNum(item.amountUSD, true)}
        </DataText>
        {!below780 && (
          <>
        ...
        {!below1080 && (
          <DataText area='account'>
            <Link color={color} external href={'https://explorer.evm.eosnetwork.com/address/' + item.account}>
