...
const SORT_FIELD = {
  VALUE: 'VALUE',
  NEUTROSWAP_RETURN: 'NEUTROSWAP_RETURN'
}
          ...
          <DataText area='return'>
            <AutoColumn gap='12px' justify='flex-end'>
              ...
              <AutoColumn gap='4px' justify='flex-end'>
                <RowFixed>
                  <TYPE.small fontWeight={400}>
                    {parseFloat(position.pair.token0.derivedEOS)
                      ? formattedNum(
                          position?.fees.sum / (parseFloat(position.pair.token0.derivedEOS) * eosPrice) / 2,
                          false,
                          true
                        )
                      : 0}{' '}
                ...
                <RowFixed>
                  <TYPE.small fontWeight={400}>
                    {parseFloat(position.pair.token1.derivedEOS)
                      ? formattedNum(
                          position?.fees.sum / (parseFloat(position.pair.token1.derivedEOS) * eosPrice) / 2,
                          false,
                          true
                        )
                      : 0}{' '}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.NEUTROSWAP_RETURN) {
        ...
        {!below500 && (
          <Flex alignItems='center' justifyContent='flexEnd'>
            <ClickableText
              area='return'
              onClick={() => {
                setSortedColumn(SORT_FIELD.NEUTROSWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.NEUTROSWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.NEUTROSWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
