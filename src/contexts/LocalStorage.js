...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(NEUTROSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(NEUTROSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
